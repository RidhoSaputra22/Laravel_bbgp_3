...
<div x-data="{ open: {{ $panelStartsOpen ? 'true' : 'false' }} }" class="fixed bottom-4 right-4 z-50">
    <button x-show="!open" x-cloak type="button" @click="open = true"
        class="relative flex items-center gap-3 rounded-xl bg-indigo-600 px-5 py-3 font-semibold text-white shadow-xl transition hover:bg-indigo-700">
        <i class="fas fa-clipboard-check"></i>
        Tugas Validasi
        @if ($pendingValidatorTaskCount > 0)
            <span class="absolute -right-2 -top-2 flex h-6 min-w-6 items-center justify-center rounded-full bg-red-500 px-1 text-xs">
    ...
    <section x-show="open" x-cloak
        class="flex max-h-[calc(100vh-2rem)] w-[min(92vw,36rem)] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <header class="flex shrink-0 items-center justify-between bg-indigo-600 px-5 py-4 text-white">
            <div>
            ...
            <button type="button" @click="open = false"
                class="rounded-lg p-2 text-indigo-100 transition hover:bg-indigo-500 hover:text-white"
        ...
        <div class="min-h-0 flex-1 overflow-y-auto p-5">
            ...
            @if ($selectedValidatorTask)
                ...
                <form method="POST" action="{{ route('assessment.portal.validator.tasks.submit', $selectedValidatorTask) }}"
                            ...
                            <div class="space-y-4">
                                @foreach ($section->fields->where('is_active', true) as $field)
                                    @php
                                        ...
                                        $inputClass = 'mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none';
                                    @endphp
                                    <div>
                                        ...
                                        @switch($field->field_type)
                                            ...
                                            @case('likert')
                                                <div class="mt-2 flex flex-wrap gap-2">
                                                    @foreach ($field->resolvedOptions() as $option)
                                                        <label class="cursor-pointer rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50 has-[:checked]:text-indigo-700">
                    ...
                    <fieldset class="rounded-xl border border-slate-200 p-4">
                        ...
                        <select name="recommendation" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                            @disabled($taskIsLocked || $taskIsUpcoming)>
                        ...
                        <textarea name="final_notes" rows="4" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
            ...
            @else
                @forelse ($validatorTasks as $task)
                    <a href="{{ route('assessment.portal.dashboard', ['validator_task' => $task->id]) }}"
                        class="mb-3 block rounded-xl border border-slate-200 p-4 transition hover:border-indigo-300 hover:bg-indigo-50">
